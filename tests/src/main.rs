...
async fn suite(cli: &Cli) -> Result<()> {
    ...
    tokio::spawn(async move {
        ingestor_start(shutdown_listener_ingestor, ingestor_settings)
            .await
            .expect("ingestor failed");
    });
    let keypair_vec = keypair.to_vec();
    tokio::spawn(async move {
        rewarder_start(shutdown_listener_rewarder, rewarder_settings, &keypair_vec)
            .await
            .expect("rewarder failed");
    });
    tokio::spawn(async move {
