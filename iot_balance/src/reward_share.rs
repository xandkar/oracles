...
use crate::{
    subnetwork_rewards::SubnetworkRewards, traits::OwnerResolver, CellType, Iot, PublicKey,
    Result,
};
...
/// Map from owner_public_key to accumulated_rewards (iot)
#[derive(Debug, Clone, Serialize)]
pub struct OwnerEmissions(HashMap<PublicKey, Iot>);
...
impl OwnerEmissions {
        ...
        if total_shares > dec!(0) {
            if let Some(actual_emissions) = get_scheduled_tokens(start, duration) {
                ...
                for (owner, share) in owner_shares {
                    owner_emissions.insert(owner, Iot::from(share * emissions_per_share));
                }
            }
        }
    ...
    pub fn total_emissions(&self) -> Iot {
        Iot::from(
            self.0
                .values()
...
mod test {
    ...
    fn emissions_per_second() {
        ...
        assert_eq!(
            Iot::from(dec!(1157.40740741)),
            Iot::from(scheduled_tokens)
        );
    }
}
